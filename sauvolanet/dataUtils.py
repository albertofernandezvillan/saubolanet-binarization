...
def get_model_path(path = 'pretrained_models/weights.h5'):
    filepath = pkg_resources.resource_filename(__name__, path)
    return filepath
    
def get_dataset_path(path = 'dataset'):
    path = pkg_resources.resource_filename(__name__, path)
    return path

def get_all_dataset_images(path = 'dataset'):
    path = pkg_resources.resource_listdir(__name__, path)
    return path

def get_weights(path = 'pretrained_models/weights.h5'):
